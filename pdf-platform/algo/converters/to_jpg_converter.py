...
def convert(file_path: str, params: ConvertParams, output_dir: str) -> str:
    ...
    将 PDF 每一页渲染为独立的 JPG 文件，写入 ``output_dir``。

    - 单页：输出 ``{base_name}.jpg``，返回该文件路径。
    - 多页：第一页输出 ``{base_name}.jpg``，其余页输出
      ``{base_name}_page_{N:03d}.jpg``，并返回 ``output_dir`` 目录路径，
      以便调用方知道存在多个结果文件。
    ...
    Args:
        ...
        params: 转换参数（使用 image.dpi / image.quality / start_page / end_page）
    ...
    # 第一页命名为 {base_name}.jpg，与单页输出保持一致
